articles = [{
    'data': '2020 01 01',
    'autorius': 'Autorius 1',
    'pavadinimas': 'Apie nieką',
    'status': 'published',
    'tekstas': 'Zombie ipsum reversus ab viral inferno, nam rick grimes malum cerebro. De carne '
               'lumbering animata corpora quaeritis. Summus brains sit​​, morbo vel maleficia? De '
               'apocalypsi gorger omero undead survivor dictum mauris.'
},
    {
        'data': '2020 02 01',
        'autorius': 'KITAS AUTORIUS',
        'pavadinimas': 'Apie zombius',
        'status': 'published',
        'tekstas': 'Zombie ipsum reversus ab viral inferno, nam rick grimes malum cerebro. De '
                   'carne lumbering animata corpora quaeritis. Summus brains sit​​, morbo vel '
                   'maleficia? De apocalypsi gorger omero undead survivor dictum mauris. '
    },
    {
        'data': '2020 03 01',
        'autorius': 'Dar kažkas',
        'pavadinimas': 'Braiiins!',
        'status': 'unpublished',
        'tekstas': 'Zombie ipsum reversus ab viral inferno, nam rick grimes malum cerebro. De '
                   'carne lumbering animata corpora quaeritis. Summus brains sit​​, morbo vel '
                   'maleficia? De apocalypsi gorger omero undead survivor dictum mauris.'
    }]